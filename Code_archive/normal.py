import math    #导入math库
import os      #导入OS模块（基本的操作系统交互模块）
import cv2     #导入cv2库（开源的库平台计算机视觉库）
import dlib    #导入dlib模块（用来检测人脸关键点）
...
# 我想检测这个人有没有打呵欠
class DetectWeary:        #类detectweary
    def __init__(self):   #初始化
        self.face_area = {
            "outline": [0, 16],           #0到16是人脸轮廓关键点
            "left_eyebrow": [17, 21],     #17到21是左眉毛
            "right_eyebrow": [22, 26],    #22到26是右眉毛
            "nose": [27, 35],             #27到35是鼻子
            "left_eye": [36, 41],         #36到41是左眼
            "right_eye": [42, 47],        #42到47是右眼
            "mouse": [48, 67]             #48到67是嘴
        }
        self.EYERATIO = 0.25
    ...
    def detect_process(self,path):
        ...
        for face in faces:
            ...
            bottom = face.bottom()
            # 切割出人脸图像
            #face_img = original_img[top:bottom, left:right]
            #im = cv2.resize(face_img, (24, 24))
        ...
        if mouse_ratio > 0.75:
            mouse = 'Yes'
        if left_eye_ratio <= 0.20:
            left_eye = 'Closed'
        if right_eye_ratio <= 0.20:
            right_eye = "Colsed"
        s = '''left eye: %s   right eye: %s '''%(left_eye,right_eye)

        #s = '''yawn: %s %s  left eye: %s %s  right eye: %s %s''' % (mouse, round(mouse_ratio, 2),left_eye, round(left_eye_ratio, 2), right_eye, round(right_eye_ratio, 2))
        print(s)
...
path = r"D:\pythonproject\DrowsyDriverDetection-master\pic\normal"
